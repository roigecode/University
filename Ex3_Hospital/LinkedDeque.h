...
public:
    LinkedDeque();
    LinkedDeque(const LinkedDeque& queue_original);
    virtual ~LinkedDeque();
...
template <class Elem>
LinkedDeque<Elem>::LinkedDeque(const LinkedDeque& queue_original) {

    // Caso 0: Copiar lista vacía:
    if (queue_original._head == nullptr) {
        _size = 0;
        _head = nullptr;
        _rear = nullptr;
        ...
        _head = new DoubleNode<Elem>(queue_original._head->getElement());
        _rear = new DoubleNode<Elem>(queue_original._rear->getElement());
        _head->setPrevious(nullptr);
        _head->setNext(_rear);
        _rear->setPrevious(_head);
        _rear->setNext(nullptr);
    }
    else {// Caso n: Lista de 'n' elementos:
        _head = new DoubleNode<Elem>(queue_original._head->getElement());
        _rear = new DoubleNode<Elem>(queue_original._rear->getElement());
        _head->setPrevious(nullptr);
        _head->setNext(_rear);
        ...
        DoubleNode<Elem>* from = queue_original._head->getNext();
        DoubleNode<Elem>* to = _head;

        while (from != queue_original._rear) {
...
void LinkedDeque<Elem>::enqueueBack(Elem& elem) {
    ...
    _size++;
    cout << "Element " << elem << " agregat" << endl;
}
...
// PRINT PARA PATIENT:
template <> // No template argument here!
void LinkedDeque<Patient*>::print() {

    if (isEmpty()) {
        cout << "[]\n" << endl;
    } else {
        cout << "[" << endl;
        DoubleNode<Patient*>* temp = _head;
