 ...
 * getHeight                          := O(n) worst case, O(log(n)) casos normals
 * getKey                             := O(1)
 * getValues                          := O(1)
 * parent                             := O(1)
 * left                               := O(1)
...
public:
    ...
    int depth() const;
    int getHeight() const;
    int getHeightRec() const; // uses auxiliary attribute
...
template <class K, class V>
void BinaryTreeNode<K, V>::setHeight(int height) {
    this->_height = height;
}

template <class K, class V>
void BinaryTreeNode<K, V>::setFDB(int FDB) {
    this->_fdb = FDB;
}
...
template <class K, class V>
int BinaryTreeNode<K, V>::getFDB() const {
    return this->_fdb;
}
...
template <class K, class V>
int BinaryTreeNode<K, V>::getHeightRec() const {
    // Cas 0 - Base: un node fulla té getHeight = 1
    if (this->isLeaf())
        return 1;
        //Cas 1: Té dos fills: calculem l'alçada màxima dels dos fills + 1
    else if (_left != nullptr && _right != nullptr)
        return 1 + max(_left->getHeight(), _right->getHeight());
        //Cas 2: Només té fill dret:
    else if (_left == nullptr)
